...
| Skill | Covers |
|---|---|
| [`tapestry-local-dev-environment`](.claude/skills/tapestry-local-dev-environment/SKILL.md) | Running the app locally the way upstream documents it (npm workspaces, per-workspace `.env` files, LocalStack/Redis/Vault via `docker-compose.local.yml`, Vite dev server) — plus a clearly-separated note on a Docker+MinIO installer variation that exists only on some forks, the CSP issue that only shows up there, and real Puppeteer-screenshot gotchas on that same installer (cookie scoping, a Chromium secure-context flag, host-resolver-rules vs. request interception) |
| [`tapestry-multi-checkout-workflow`](.claude/skills/tapestry-multi-checkout-workflow/SKILL.md) | Working across several open `tapestry-project` PRs at once with one checkout (or `git worktree`) per PR, each intentionally parked on its own branch — verified gotchas on remote-naming inconsistency between checkouts, a worktree's `.git` living in a different directory than its working tree, and syncing a fork's `main` without destroying its own commits |
...
| [`tapestry-content-types`](.claude/skills/tapestry-content-types/SKILL.md) | Adding a new canvas content/item type while changing as little as possible — full checklist including the easy-to-miss export-version bump, generalized from two real reference implementations: IIIF deep-zoom images (unmerged as [PR #77](https://github.com/asteasolutions/tapestry-project/pull/77), later reworked around a full third-party viewer library as [PR #123](https://github.com/asteasolutions/tapestry-project/pull/123)) and STL 3D models — plus a variation for an *existing* type accepting a format that needs conversion before it's renderable, with client-side vs. server-side tradeoffs verified against two real, competing PRs (HEIC image import) |
...
| [`tapestry-external-media-sources`](.claude/skills/tapestry-external-media-sources/SKILL.md) | Letting users paste a URL that *describes* a single file on an external platform (e.g. a Wikimedia Commons `File:` page) and importing the real file as a plain, ordinary item of an existing type — no schema changes at all, the lightest of the "URL connection" patterns. Openverse (image/audio) and Wikimedia Commons (image/video/audio/pdf) are now real, verified implementations, [PR #112](https://github.com/asteasolutions/tapestry-project/pull/112) (through two full review rounds) — including a real browser-codec-compatibility gotcha (prefer WebM/MP3 transcodes over Ogg originals), why Commons' `videoinfo` endpoint is queried for every file type despite its name, and a corrected finding: real review rejected this skill's own earlier "route through a server-side proxy by default" recommendation once CORS was confirmed and the real fix turned out to be disabling an unrelated background-reload default, not server-side infrastructure |
| [`tapestry-collection-imports`](.claude/skills/tapestry-collection-imports/SKILL.md) | Extending Tapestries' existing bulk-import picker (real upstream functionality) with a new collection type — a Wikimedia Commons category, an Openverse tag/source search, an Internet Archive search query — generalized from two real implementations against actual upstream: IA search ([PR #96](https://github.com/asteasolutions/tapestry-project/pull/96)) and Openverse+Wikimedia Commons together in one PR ([PR #112](https://github.com/asteasolutions/tapestry-project/pull/112), through two full review rounds), including what genuinely generalizes across platforms vs. what has to stay separate (cursor vs. numeric pagination), a shared-component "clears itself on any failure" bug, a concrete recipe for adding a third platform, and real review corrections to the original design across both rounds (direct union members instead of a platform-nested one, no server-side proxy, a shared `CollectionList` shell — then a later round splitting the per-platform container built on it back apart once real type differences accumulated, since a merge isn't permanent) |
...
| [`tapestry-pr-conventions`](.claude/skills/tapestry-pr-conventions/SKILL.md) | Code-review conventions actually observed from real maintainers, now verified across nine-plus review rounds on four real PRs (#96, #109, #123, #112) plus one direct follow-up question from a reviewer — comment discipline, merge-don't-duplicate (business logic and shared UI components alike), composition over internal dependency, trust the strongest already-available signal (but check its boundary conditions), match the full input space of the pipeline you're plugging into, distinguish a missing value from a meaningless-but-present one, prefer a documented known limitation over a partial workaround for a rare upstream bug, verify a suspected dependency-declaration gap against the real `package.json`, replace ternary sprawl with a per-branch config function, reject a server-side proxy once CORS and a simpler client-side fix are confirmed, give a discriminated union direct members instead of nesting a platform inside one, rename a type once its scope outgrows its name, re-check a past merge for real structural drift that means it should un-merge — plus a pre-submission self-review checklist, the exact `gh`/GraphQL commands (with a real empty-review-body gotcha and a re-check-before-assuming-a-thread-is-pending gotcha), and a project-standing (not reviewer-observed) ASD-STE100 writing-style rule for comments and PR replies |
...
- IIIF's own reference implementation later moved from "unmerged fork branch" to "real
  open PR": [PR #77](https://github.com/asteasolutions/tapestry-project/pull/77) (the
  original hand-rolled single-canvas OpenSeadragon wrapper this skill was first built
  from) was superseded by [PR #123](https://github.com/asteasolutions/tapestry-project/pull/123),
  which reworked the viewer to render the whole manifest via `@samvera/clover-iiif`'s
  full `<Viewer>` component instead. Real review on #123 rejected a workaround I'd added
  for a genuine OpenSeadragon bug (accept it as a documented limitation instead — folded
  into `tapestry-pr-conventions` point 18) and caught an assumption about a transitive
  dependency's declared dependencies that turned out to be simply wrong once checked
  against its actual `package.json` (point 19). Both are now general guardrails in this
  skill and in `tapestry-pr-conventions`, not just PR-specific trivia.
...
- Both of those two skills' own "route through a server-side proxy by default" advice
  was real, own-conclusion guidance (not reviewer-observed) that turned out to be
  wrong, corrected once [PR #112](https://github.com/asteasolutions/tapestry-project/pull/112)
  reached real review: `zmarinov-astea` rejected the proxy outright, both platforms'
  APIs were confirmed CORS-enabled by direct request, and the actual rate-limit risk
  traced to an unrelated background-reload default on a shared list component, not to
  the resolution calls the proxy existed to protect. A synthetic burst test's real
  finding (both platforms do rate-limit a 20-concurrent-request burst) was correct;
  the conclusion drawn from it (therefore proxy by default) was not, since normal usage
  never produces that burst. Left in both skills as a corrected recommendation with the
  original reasoning still visible, rather than quietly erased — the mistake itself
  (a real burst-test finding overextended into an architectural default) is worth a
  future editor seeing, not just its fix. Same PR also drove three real corrections in
  `tapestry-collection-imports`' original multi-platform design (direct union members
  instead of one nested by `platform`, a shared `CollectionList` UI component instead of
  one component parameterized further, and two new `tapestry-pr-conventions` points on
  ternary sprawl and renaming a type once its scope outgrows its name).
