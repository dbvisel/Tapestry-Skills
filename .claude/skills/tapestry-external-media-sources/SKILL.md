...
Generalized from two real, complete reference implementations on an unmerged, messy fork
branch (`dbvisel/tapestry-project` branch `wikimania-mess`, one giant "Everything added for
Wikimania" commit — only the lines relevant to this pattern were used):
...
single-item case. A future skill can cover the picker/bulk-import mechanism if needed.
...
5. **This skill is single-item only.** Recognizing a *collection*/category/search-results
   ...
   `HandleIAImportDialog`) intentionally left out of scope here.
6. See `tapestry-content-types` for the `core/` module conventions (defensive JSON
