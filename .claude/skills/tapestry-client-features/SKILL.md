...
| `viewer` | A separate, small, **standalone** app — no `tapestry-shared`, no `axios`/`socket.io-client`, no auth. Loads a tapestry from a `source=` URL or local IndexedDB blob, renders it read-only using `core-client`'s default components. This is what the exported `.zip` (via `client`'s export button → `TapestryExporter`) is meant to be opened with — an offline/standalone single-tapestry viewer, distinct from the full app. Its architectural minimalism is exactly what makes it embeddable in non-website hosts — see `tapestry-viewer-embedding`. |
